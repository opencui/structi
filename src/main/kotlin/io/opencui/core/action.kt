...
interface Action: Serializable {
    fun run(session: UserSession): Flow<SystemEvent>

    fun wrappedRun(session: UserSession) : Flow<SystemEvent> {
...
data class StartFill(
    ...
    override fun run(session: UserSession): Flow<SystemEvent> {
        // start every action with consuming corresponding events
        match.triggered = true
        match.typeUsed = true

        val intent = buildIntent.invoke(session) ?: return flowOf(emptyLog())
        ...
        if (session.inKernelMode(session.schedule)) return flowOf(emptyLog())
    ...
    fun findValueAndType(match: FrameEvent): Triple<String, List<String>, Int?>? {
        ...
        if (oldValueSlot != null) {
            val oldValueTypeCandidates = match.frames.firstOrNull { it.slots.firstOrNull { slot -> slot.attribute == oldValue } != null }?.slots?.map { it.type!! }
        ...
        if (newValueSlot != null) {
            val newValueTypeCandidates = match.frames.firstOrNull { it.slots.firstOrNull { slot -> slot.attribute == newValue } != null }?.slots?.map { it.type!! }
...
data class SimpleFillAction(
    ...
    override fun run(session: UserSession): Flow<SystemEvent> {
        ...
        if (!success) return flowOf(emptyLog(false))

        session.schedule.state = Scheduler.State.RESCHEDULE
        return flowOf(emptyLog(true))
    }
}
...
data class RefocusAction(
    ...
    override fun run(session: UserSession): Flow<SystemEvent> {
        ...
        if (scheduler == null) return flowOf(emptyLog())
        ...
        return if (scheduler == session.schedule) {
...
data class SlotAskAction(val tag: String = "") : StateAction {
    ...
    override fun run(session: UserSession): Flow<SystemEvent> {
        ...
        return flow {
            val allEvents = res.toList()
            val actionLogs = allEvents.filterIsInstance<SystemEvent.ActionStatus>()

            val status = if (actionLogs.isNotEmpty()) {
                val firstLog = actionLogs.first()
                if (firstLog.payload is ArrayNode) {
                    createLog(firstLog.payload.filterIsInstance<ObjectNode>().joinToString("\n") { it["payload"].asText() }, firstLog.success)
                } else {
                    createLog(firstLog.payload, firstLog.success)
                }
            } else {
                emptyLog()
            }
            emit(status)
            allEvents.forEach { if (it is DialogAct) emit(it) }
        }
    }
}
...
data class SlotPostAskAction(
    ...
    private fun goback(session: UserSession): Flow<SystemEvent> {
        ...
        return delegateActionResult ?: flowOf(emptyLog())
    }
...
class RespondAction : CompositeAction {
    override fun run(session: UserSession): Flow<SystemEvent> {
        ...
        val res = if (response != null) {
            ...
            runBlocking {
                val status = tmp.filterIsInstance<SystemEvent.ActionStatus>().firstOrNull()
                if (status != null && !status.success) {
                    throw Exception("fail to respond!!!")
                }
            }
            tmp
        } else {
...
class RescheduleAction : StateAction {
    override fun run(session: UserSession): Flow<SystemEvent> {
        ...
        while (schedule.isNotEmpty()) {
            ...
            schedule.parentGrandparentBothAnnotated()
            if (topDone || (top.isForInterfaceOrMultiValue() && topParentDone)) {
                ...
                if (doneFiller is AnnotatedWrapperFiller && doneFiller.isSlot) {
                    ...
                    logger.debug("Reschedule ends with result: ${runBlocking { result.toList() }}")
                    return result
                }
            } else {
                break
...
abstract class ExternalAction(
    ...
    override fun run(session: UserSession): Flow<SystemEvent> {
        try {
            ...
            if (result != null) {
                return flow {
                    val allEvents = result.toList()
                    val actionLogs = allEvents.filterIsInstance<SystemEvent.ActionStatus>()
                    val jsonArrayLog = mutableListOf<JsonElement>()
                    jsonArrayLog.add(Json.makePrimitive("EXTERNAL ACTION : ${action.javaClass.name}"))
                    actionLogs.forEach { jsonArrayLog.add(Json.encodeToJsonElement(it)) }
                    val log = createLog(Json.makeArray(jsonArrayLog), true)
                    emit(log)
                    allEvents.forEach { if (it is DialogAct) emit(it) }
                }
            }
...
data class IntentAction(
    ...
    override fun run(session: UserSession): Flow<SystemEvent> {
        ...
        return flowOf(createLog("INTENT ACTION cannot construct intent : $jsonIntent", false))
        ...
        return flowOf(createLog("INTENT ACTION : ${intent.javaClass.name}", true))
    }
}
...
open class SeqAction(val actions: List<Action>): CompositeAction {
    ...
    override fun run(session: UserSession): Flow<SystemEvent> = channelFlow {
        val logs = mutableListOf<SystemEvent.ActionStatus>()
        var flag = true

        for (action in actions) {
            action.wrappedRun(session).collect { event ->
                when (event) {
                    is DialogAct -> send(event)
                    is SystemEvent.ActionStatus -> {
                        logs += event
                        flag = flag && event.success
                    }
                    else -> {}
                }
            }
        ...
        val log = createLog(Json.makeArray(logs.map { l -> Json.encodeToJsonElement(l) }), flag)
        send(log)
        Dispatcher.logger.info("got the following messages: ${logs.toString()}")
    }
}
...
class Handoff: EmissionAction {
    ...
    override fun run(session: UserSession): Flow<SystemEvent> {
        ...
        return flowOf(createLog("HandOff", true))
    }
