...
// All agent are reducers, to work with multiple agents, we need to know when it is done.
data class ReduceState(val ready: Boolean, val question: String? = null, val reason: String? = null)
data class RelatedStatus(val related: Boolean, val reason: String? = null)

// Most of the frame/skills process event generated by DU, but some frames are the LLM based, thus can
// handle raw input or utterance directly. If a frame is IRawInputHandler, then none of its slots can be
// filled by DU events, (can be system events),
// These skills/frames still need to be triggered with event created by dialog understanding, so builder need
// to provide exemplars if needed. This always need to be the first slot?, and last one is reducestate.
interface IRawInputHandler : Serializable {
    var rawUserInput: String?
}

// For the LlmReducer, we need to make sure that rawUserInput is the first one, and ReduceState is last one,
// as all of the 
interface LlmReducer: IRawInputHandler {
    var decision: ReduceState
    fun isRelated(question: String): RelatedStatus { return RelatedStatus(true) }
    fun check(): ReduceState
}
